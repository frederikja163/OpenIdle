...
<Column class="mx-auto w-full max-w-[1600px] grow gap-(--gutter-app) p-(--gutter-app)">
	<Row class="items-baseline gap-(--sp-5)">
		...
		<span class="oi-body-md text-text-muted">
			...
			{profilesState.profiles.length === 1 ? 'profile' : 'profiles'}
		</span>
	...
	{/if}
	<!--
		No items-start: the grid's default stretch is what keeps a profile card and
		the dashed create card the same height. The obvious fix for the space under a
		profile card's footer is to add it, which silently breaks that match.
	-->
	<div
		class="grid grid-cols-[repeat(auto-fill,minmax(360px,1fr))] content-start gap-(--gutter-app)"
	>
		{#each profilesState.profiles as profile (profile.profileId)}
			<ProfileCard
				{profile}
				selected={profile.profileId === profilesState.selectedProfileId}
