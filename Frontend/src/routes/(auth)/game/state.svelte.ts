...
import { gameState, startActivity, stopActivity } from '$lib/state/game.svelte';
...
export class BoardState {
	...
	readonly actions: Record<PlayableSkillId, GameAction[]> = actionsBySkill;
	...
	held = $derived(gameState.items);

	running = $derived.by<RunningAction | null>(() => {
		const current = gameState.running;
		const action = current && actionById(current.activityId);
		return action ? { skill: action.skill, action: action.id, ms: action.ms, xp: action.xp } : null;
	});
	...
	progress = $derived.by(() => {
		...
		return Math.min(100, ((this.#now - current.startedAt) / this.running.ms) * 100);
	});

	reward = $derived(gameState.lastReward);
