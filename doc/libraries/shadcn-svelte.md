...
**The tree is the deepest in the project.** Eight direct additions (`shadcn-svelte`, `bits-ui`, `clsx`, `tailwind-merge`, `tailwind-variants`, `tw-animate-css`, `@lucide/svelte`, `@fontsource-variable/inter`, plus the peer `@internationalized/date`) pull roughly eleven transitives, enumerated in [bits-ui](./bits-ui.md). One qualification: [clsx](./clsx.md) was **already in the tree** as a dependency of [Svelte](./svelte.md) itself — the CLI promoted it to a direct declaration but did not introduce it, so it is not new exposure. Licensing was checked across all of them: MIT throughout except `@lucide/svelte` (ISC), `@internationalized/date` (Apache-2.0) and `@fontsource-variable/inter` (OFL-1.1) — all compatible. (`@fontsource-variable/inter` was uninstalled on 2026-08-04; the three OFL-1.1 typefaces that replaced it are listed in the [typefaces section](./README.md#typefaces). The licence picture is unchanged.) `svelte-toolbelt` declares **no `license` field in its `package.json`**, though its bundled LICENSE file is MIT under the same maintainer as shadcn-svelte itself. Benign, but it is a metadata gap in a project that tracks licence terms, and automated licence tooling will flag it.
