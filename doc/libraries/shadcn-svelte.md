...
Those figures were measured during evaluation. **`button`, `badge`, `card`, `input` and `dialog` are now vendored** under `src/lib/components/ui/`, each re-tabled against the design system, so both costs above are being paid. Re-measured on the app as it actually stands, adding `dialog` — and with it [bits-ui](./bits-ui.md), which now genuinely reaches the browser — moved total client JavaScript from **67.6 KB to 82.5 KB gzip, +14.9 KB**, a little under the 17.2 KB the evaluation predicted. Components continue to be added on demand with `shadcn-svelte add`.

`input` is the first vendored component with no design-system counterpart to transcribe from — the design has a Button, a Panel and a Badge, but no Input. It is re-tabled against Button's vocabulary instead (the same control padding, the recessed `--inset-well` surface, and Button's offset-outline focus rather than shadcn's ring), and the `type="file"` branch is dropped as an unused variant. That substitution is the reason the profiles page's create form was kept inline rather than put in a `dialog`: at the time, a dialog would have made [bits-ui](./bits-ui.md) reach the browser for the first time, which is a bigger decision than a text field warranted.

That threshold has since been crossed deliberately, by the delete-confirmation modal on the profile card. The create form stays inline — the reasoning above still holds for a single text field, and there is no reason to spend a modal on it now that one is available. What changed is only that a destructive action was judged to be worth the dialog that a text field was not. `dialog` needed two edits the generator could not know about: it imports `WithoutChildrenOrChild` from our hand-maintained `stylingUtils.ts`, which had to grow that type, and its close button asks for `size="icon-sm"` and its footer for `variant="outline"`, neither of which survives in Button's fork.

The first component is the expensive one: `tailwind-merge` and `tailwind-variants` are a fixed cost paid on first use, not per component. Adding the dialog — and with it [bits-ui](./bits-ui.md) — cost a further 14.9 KB when it actually happened. Doubling baseline client JavaScript for two components is a real price, it is being paid knowingly, and for an idle game whose sessions are long and whose bundle is cached after first load it is judged acceptable. It would not be acceptable for a landing page.
