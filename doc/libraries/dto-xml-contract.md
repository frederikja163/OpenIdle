...
- `Property` attributes:
  ...
  - `type` — required. Tokens: `string`, `int`, `float`, `Guid`, or the bare name of another contract type (which becomes `{X}Dto`, e.g. `Profile` → `ProfileDto`). Unrecognized tokens are treated as custom types, so `type="Gud"` silently becomes `GudDto` — a typo trap documented in §3.
...
- The base classes are generated in the same file: `DtoBase`, `RequestBase : DtoBase` and `ResponseBase : DtoBase` (each with `string RequestId { get; set; }`), `EventBase : DtoBase` (with `EventId`).
...
- `interface DtoBase { $type: string; }`; `RequestBase`/`ResponseBase` each with `requestId: string;`; `EventBase` with `eventId: string;`; then per object `interface {Name} extends {Base}` with `{key}: {type};` per property.
...
- **Frontend handshake** — how generated TS reaches `Frontend/` (checked-in file vs a build step) is unsettled; today it is a manual CLI run.
