...
- ~~**Frontend handshake**~~ — *resolved 2026-08-30*: the generated TS reaches `Frontend/` through a wired build step, not a checked-in file. Locally, `Frontend/package.json`'s `generate` script runs both emitters — `gen:dto` for `src/lib/ws/dto.generated.ts` and `gen:schema` for `src/lib/debug/protocol.generated.ts` — before `dev`, `build` and `check`, so the TypeScript is regenerated from `types.xml` every time the frontend starts and can never be stale against the contract. **A .NET SDK is therefore a hard prerequisite for frontend development.** In the Docker build, `Frontend/Dockerfile` regenerates the same files in a .NET SDK stage from the same commit's `types.xml` (see [deployment](../deployment.md)). Generated TS files use the `*.generated.ts` pattern, which the root `.gitignore` rule `**/*.generated.*` excludes at any depth — including under `Frontend/` — so the output is regenerated on demand rather than committed.
