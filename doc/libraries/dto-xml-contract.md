...
- ~~**Frontend handshake**~~ — **settled 2026-08-21.** `bun run dev`, `bun run build` and `bun run check` all run `gen:dto` first, so the TypeScript is regenerated from `types.xml` every time the frontend starts and can never be stale against the contract. **A .NET SDK is therefore a hard prerequisite for frontend development.** The output at `Frontend/src/lib/ws/dto.generated.ts` is *also* committed — the root `.gitignore` keeps `**/*.generated.*` but re-includes that one path — so protocol changes show up in review and the backend CI job can fail on a diff. Committing it is redundant for the build but not for the diff.
