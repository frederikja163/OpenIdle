...
- A `// <auto-generated>` header, then `export interface DtoBase { $type: string; }`, with `RequestBase`/`ResponseBase` (`requestId?: number`) and `EventBase` (`eventId?: number`) extending it. Enums become unions of string literals (the backend registers `JsonStringEnumConverter`, so enums travel as strings). Each request/response/event is an `export interface {Name} extends {Base}` whose `$type` is narrowed to its own literal; a `<Dto>` gets neither `$type` nor a base, because `[JsonPolymorphic]` is not inherited and a nested DTO carries no discriminator on the wire.
