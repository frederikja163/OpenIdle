...
Generated TypeScript (via `bun run gen:dto`, or `dotnet run --project Generators/Generator -- -i types.xml -t Ts -o Frontend/src/lib/ws/dto.generated.ts`):

- A `// <auto-generated>` header, then `export interface DtoBase { $type: string; }`, with `RequestBase`/`ResponseBase` (`requestId?: number`) and `EventBase` (`eventId: number`, `timestamp: number`, both stamped by the backend at send) extending it. Enums become unions of string literals (the backend registers `JsonStringEnumConverter`, so enums travel as strings). Each request/response/event is an `export interface {Name} extends {Base}` whose `$type` is narrowed to its own literal; a `<Dto>` gets neither `$type` nor a base, because `[JsonPolymorphic]` is not inherited and a nested DTO carries no discriminator on the wire.
- The emitter also derives what the socket client is generic over — `ServerResponse`, the `RESPONSE_TYPES` runtime set, `RequestMap`, `EventMap` and `ServerEventOf` — plus the `<DropTable>`/`<Activity>` data as `DROP_TABLE_DATA`/`ACTIVITY_DATA` constants (each activity carries its `time` in seconds and its `costs`, which is how the game board draws a progress meter and locks a card behind materials without any of it crossing the wire). Output is formatted to satisfy `Frontend/prettier.config.js` unchanged.
...
- ~~**Frontend handshake**~~ — *resolved 2026-08-30*: the generated TS reaches `Frontend/` through a wired build step, not a checked-in file. Locally, `Frontend/package.json`'s `generate` script runs the emitter — `gen:dto` for `src/lib/ws/dto.generated.ts` — before `dev`, `build` and `check`, so the TypeScript is regenerated from `types.xml` every time the frontend starts and can never be stale against the contract. **A .NET SDK is therefore a hard prerequisite for frontend development.** In the Docker build, `Frontend/Dockerfile` regenerates the same file in a .NET SDK stage from the same commit's `types.xml` (see [deployment](../deployment.md)). What needs the contract at runtime rather than at compile time — the debug console's request forms — fetches it from the backend's `GET /schema` instead. Generated TS files use the `*.generated.ts` pattern, which the root `.gitignore` rule `**/*.generated.*` excludes at any depth — including under `Frontend/` — so the output is regenerated on demand rather than committed.
