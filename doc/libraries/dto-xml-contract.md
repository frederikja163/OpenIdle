...
- The emitter also derives what the socket client is generic over — `ServerResponse`, the `RESPONSE_TYPES` runtime set, `RequestMap`, `EventMap` and `ServerEventOf` — plus the `<DropTable>`/`<Activity>` data as `DROP_TABLE_DATA`/`ACTIVITY_DATA` constants. Output is formatted to satisfy `Frontend/prettier.config.js` unchanged.
