...
| A component library (DaisyUI, Skeleton) | varies | Pre-built components, consumed as a package | Varies | MIT | Low: a game UI is heavily custom; imported generic components would be fought, not used |
| ~~shadcn-svelte~~ — **superseded, see below** | 1.5.0 | Copies component *source* into the repo rather than exporting a package | Active | MIT | Reassessed 2026-08-03 and **adopted** as a complement, not a replacement — see [shadcn-svelte](./shadcn-svelte.md) |

**Correction (2026-08-03).** This table originally grouped shadcn-svelte with DaisyUI and Skeleton and rejected all three as "generic components would be fought, not used". That reasoning holds for DaisyUI and Skeleton, whose components are imported from a package, but was **wrong about shadcn-svelte**, which is a code generator rather than a component library: its CLI writes component source into `src/`, where it becomes ordinary project files we own and edit. There is no package boundary to fight. shadcn-svelte was subsequently adopted — see [shadcn-svelte](./shadcn-svelte.md) and [bits-ui](./bits-ui.md) — on the strength of accessible interactive primitives, not pre-built styling. It does not displace Tailwind; it is built on it, and the game UI proper is still hand-written against Tailwind's scale.

Why the others lost: an *imported* component library is the wrong shape for a bespoke game interface, and BEM solves a problem Svelte already solves. The real contest was against Svelte's own scoped CSS plus a hand-written token block — genuinely close, and decided in section 3 on the grounds that a hand-rolled token set is a convention rather than a constraint, and that it leaves the per-component naming and context-switching cost untouched.
