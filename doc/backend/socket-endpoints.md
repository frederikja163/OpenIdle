...
- **Errors:** any exception thrown inside the handler (or during deserialization) is caught in `Backend/Socket.cs:102-105` and converted into a single `ErrorResponse` frame with the exception message. There is no structured error payload today — the message text is the contract. Throwing is the intended way to reject a request.
