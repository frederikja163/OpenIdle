...
| Document | What it answers | Read before |
|---|---|---|
| [`deployment.md`](./deployment.md) | The four hosted environments, the two GHCR images and their tags, the build info stamped into them (what the version footer shows), the GitHub Actions pipeline and its secrets, how the frontend picks a backend (`PUBLIC_WS_URL`, `PUBLIC_API_URL` and the `?ws=` override), and the WebSocket origin allowlist | any change to Dockerfiles, `.github/workflows/`, `deploy/`, how a client reaches a backend, or the version footer |
...
- **Backend:** C#/.NET, ASP.NET Core (Minimal APIs + DI), EF Core + SQLite, the in-house **DTO XML contract** (`dto-xml-contract.md`), and CommandLineParser (dev-tooling only). Identity uses self-hosted **Zitadel** with the implemented and registered [Microsoft IdentityModel](./libraries/microsoft-identitymodel.md) JWT/OIDC validator — see [`libraries/identity-provider.md`](./libraries/identity-provider.md). Full decision tables in [`libraries/README.md`](./libraries/README.md#backend).
